scalogram.extract_by_extent <- function(raster_file, shapefile_file, x) {
  ...
  for (i in seq_along(radii)) {
    ...
    yPlus <- shapefile$lat + radius*res(data)[1]
    xPlus <- shapefile$lon + radius*res(data)[1]
    yMinus <- shapefile$lat - radius*res(data)[1]
    xMinus <- shapefile$lon - radius*res(data)[1]
